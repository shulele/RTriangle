...
/*   compiler is smarter, feel free to replace the "long" with "void".        */
/*   Not that it matters.                                                    */
...
struct osub {
  subseg *ss;
  long ssorient;                                       /* Ranges from 0 to 1. */
};
...
struct event {
  ...
  long heapposition;              /* Marks this event's position in the heap. */
};
...
/* Other variables. */
  ...
  long invertices;                               /* Number of input vertices. */
  ...
  long undeads;    /* Number of input vertices that don't appear in the mesh. */
  long edges;                                     /* Number of output edges. */
  long mesh_dim;                                /* Dimension (ought to be 2). */
  long nextras;                           /* Number of attributes per vertex. */
  long eextras;                         /* Number of attributes per triangle. */
  long hullsize;                          /* Number of edges in convex hull. */
  long steinerleft;                 /* Number of Steiner points not yet used. */
  long vertexmarkindex;         /* Index to find boundary marker of a vertex. */
  long vertex2triindex;     /* Index to find a triangle adjacent to a vertex. */
  ...
  long checkquality;                  /* Has quality triangulation begun yet? */
  long readnodefile;                           /* Has a .node file been read? */
...
/* Read the instructions to find out the meaning of these switches.          */
  ...
  long poly, refine, quality, vararea, fixedarea, usertest;
  long regionattrib, convex, weighted, jettison;
  long firstnumber;
  long edgesout, voronoi, neighbors, geomview;
  long nobound, nopolywritten, nonodewritten, noelewritten, noiterationnum;
  long noholes, noexact, conformdel;
  long incremental, sweepline, dwyer;
  long splitseg;
  long docheck;
  long quiet, verbose;
  long usesegments;
...
long plus1mod3[3] = {1, 2, 0};
long minus1mod3[3] = {2, 0, 1};
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (long) ((uintptr_t) (ptr) & (uintptr_t) 3l);         \
  (otri).tri = (triangle *)                                                   \
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (long) ((uintptr_t) (sptr) & (uintptr_t) 1l);      \
  (osub).ss = (subseg *)                                                      \
...
#define mark(osub)  (* (long *) ((osub).ss + 8))

#define setmark(osub, value)                                                  \
  * (long *) ((osub).ss + 8) = value
...
#define vertexmark(vx)  ((long *) (vx))[m->vertexmarkindex]

#define setvertexmark(vx, value)                                              \
  ((long *) (vx))[m->vertexmarkindex] = value

#define vertextype(vx)  ((long *) (vx))[m->vertexmarkindex + 1]

#define setvertextype(vx, value)                                              \
  ((long *) (vx))[m->vertexmarkindex + 1] = value
...
long triunsuitable(vertex triorg, vertex tridest, vertex triapex, TRIREAL area)
#else /* not ANSI_DECLARATORS */
long triunsuitable(triorg, tridest, triapex, area)
...

{
  ...
  /* memptr = (VOID *) malloc((unsigned long) size); */
...
#ifdef ANSI_DECLARATORS
void parsecommandline(long argc, char **argv, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    ...
    if (argv[i][0] == '-') {
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'S') {
          ...
          while ((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) {
            j++;
            b->steiner = b->steiner * 10 + (long) (argv[i][j] - '0');
          }
        }
#endif /* not CDT_ONLY */
#ifndef REDUCED
  ...
  if (increment > 0) {
    ...
    do {
      if ((workstring[j] >= '0') && (workstring[j] <= '9')) {
        meshnumber = meshnumber * 10 + (long) (workstring[j] - '0');
      } else {
        increment = 0;
      }
      j++;
...
#ifdef ANSI_DECLARATORS
void poolinit(struct memorypool *pool, long bytecount, long itemcount,
              long firstitemcount, long alignment)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  pool->firstblock = (VOID **)
    trimalloc(pool->itemsfirstblock * pool->itembytes + (long) sizeof(VOID *) +
              pool->alignbytes);
...
#ifdef ANSI_DECLARATORS
void dummyinit(struct mesh *m, struct behavior *b, long trianglebytes,
               long subsegbytes)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  /*   a sizeof(long)-byte address.                                          */
  m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(TRIREAL) +
                        sizeof(long) - 1) /
                       sizeof(long);
  vertexsize = (m->vertexmarkindex + 2) * sizeof(long);
  if (b->poly) {
...

{
  ...
  if (b->usesegments) {
    ...
    poolinit(&m->subsegs, 8 * sizeof(triangle) + sizeof(long),
...
#ifdef ANSI_DECLARATORS
vertex getvertex(struct mesh *m, struct behavior *b, long number)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
long fast_expansion_sum_zeroelim(long elen, TRIREAL *e, long flen, TRIREAL *f, TRIREAL *h)
#else /* not ANSI_DECLARATORS */
long fast_expansion_sum_zeroelim(elen, e, flen, f, h)  /* h cannot be e or f. */
long elen;
TRIREAL *e;
long flen;
TRIREAL *f;
...
#ifdef ANSI_DECLARATORS
long scale_expansion_zeroelim(long elen, TRIREAL *e, TRIREAL b, TRIREAL *h)
#else /* not ANSI_DECLARATORS */
long scale_expansion_zeroelim(elen, e, b, h)   /* e and h cannot be the same. */
long elen;
TRIREAL *e;
TRIREAL b;
TRIREAL *h;
...
#ifdef ANSI_DECLARATORS
TRIREAL estimate(long elen, TRIREAL *e)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  long at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
...
void findcircumcenter(struct mesh *m, struct behavior *b,
                      ...
                      vertex circumcenter, TRIREAL *xi, TRIREAL *eta, long offcenter)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
uintptr_t randomnation(unsigned long choices)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
long checkseg4encroach(struct mesh *m, struct behavior *b,
                      struct osub *testsubseg)
#else /* not ANSI_DECLARATORS */
long checkseg4encroach(m, b, testsubseg)
struct mesh *m;
struct behavior *b;
struct osub *testsubseg;
#endif /* not ANSI_DECLARATORS */
  ...
  while (totalsamplesleft > 0) {
    ...
    do {
      sampletri.tri = (triangle *) (firsttri +
                                    (randomnation((unsigned long) population) *
...
long subsegmark;                            /* Marker for the new subsegment. */
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void vertexsort(vertex *sortarray, long arraysize)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  pivot = (long) randomnation((unsigned long) arraysize);
...
#ifdef ANSI_DECLARATORS
void vertexmedian(vertex *sortarray, long arraysize, long median, long axis)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void alternateaxes(vertex *sortarray, long arraysize, long axis)
#else /* not ANSI_DECLARATORS */
...
void mergehulls(struct mesh *m, struct behavior *b, struct otri *farleft,
                ...
                struct otri *farright, long axis)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  sortarray = (vertex *) trimalloc(m->invertices * (long) sizeof(vertex));
...
#ifdef ANSI_DECLARATORS
void eventheapinsert(struct event **heap, long heapsize, struct event *newevent)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void eventheapify(struct event **heap, long heapsize, long eventnum)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void eventheapdelete(struct event **heap, long heapsize, long eventnum)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *eventheap = (struct event **) trimalloc(maxevents *
                                           (long) sizeof(struct event *));
  *events = (struct event *) trimalloc(maxevents * (long) sizeof(struct event));
  traversalinit(&m->vertices);
  for (i = 0; i < m->invertices; i++) {
...
#ifdef ANSI_DECLARATORS
long rightofhyperbola(struct mesh *m, struct otri *fronttri, vertex newsite)
#else /* not ANSI_DECLARATORS */
long rightofhyperbola(m, fronttri, newsite)
struct mesh *m;
...
void check4deadevent(struct otri *checktri, struct event **freeevents,
                     struct event **eventheap, long *heapsize)
#else /* not ANSI_DECLARATORS */
...
struct splaynode *frontlocate(struct mesh *m, struct splaynode *splayroot,
                              ...
                              struct otri *searchtri, long *farright)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
long reconstruct(struct mesh *m, struct behavior *b, long *trianglelist,
                TRIREAL *triangleattriblist, TRIREAL *trianglearealist,
                long elements, long corners, long attribs,
                long *segmentlist,long *segmentmarkerlist, long numberofsegments)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  m->inelements = (long) strtol(stringptr, &stringptr, 0);
  stringptr = findfield(stringptr);
  if (*stringptr == '\0') {
    incorners = 3;
  } else {
    incorners = (long) strtol(stringptr, &stringptr, 0);
    if (incorners < 3) {
  ...
  if (*stringptr == '\0') {
    ...
    m->eextras = (long) strtol(stringptr, &stringptr, 0);
  }
#endif /* not TRILIBRARY */
  ...
  if (b->poly) {
    ...
    segmentmarkers = segmentmarkerlist != (long *) NULL;
#else /* not TRILIBRARY */
    ...
    m->insegments = (long) strtol(stringptr, &stringptr, 0);
    stringptr = findfield(stringptr);
    if (*stringptr != '\0') {
      segmentmarkers = (long) strtol(stringptr, &stringptr, 0);
    }
#endif /* not TRILIBRARY */
  ...
  if (b->vararea) {
    ...
    areaelements = (long) strtol(stringptr, &stringptr, 0);
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        corner[j] = (long) strtol(stringptr, &stringptr, 0);
        if ((corner[j] < b->firstnumber) ||
            (corner[j] >= b->firstnumber + m->invertices)) {
          printf("Error:  Triangle %ld has an invalid vertex index.\n",
                 elementnumber);
    ...
    for (j = 3; j < incorners; j++) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr != '\0') {
        killvertexindex = (long) strtol(stringptr, &stringptr, 0);
#endif /* not TRILIBRARY */
  ...
  if (b->poly) {
    ...
    while (subsegloop.ss != (subseg *) NULL) {
      ...
      if (*stringptr == '\0') {
        ...
        end[0] = (long) strtol(stringptr, &stringptr, 0);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        end[1] = (long) strtol(stringptr, &stringptr, 0);
      }
      if (segmentmarkers) {
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          ...
          boundmarker = (long) strtol(stringptr, &stringptr, 0);
        }
      }
#endif /* not TRILIBRARY */
      for (j = 0; j < 2; j++) {
...
#ifdef ANSI_DECLARATORS
long scoutsegment(struct mesh *m, struct behavior *b, struct otri *searchtri,
                 vertex endpoint2, long newmark)
#else /* not ANSI_DECLARATORS */
long scoutsegment(m, b, searchtri, endpoint2, newmark)
struct mesh *m;
struct behavior *b;
struct otri *searchtri;
vertex endpoint2;
...
void conformingedge(struct mesh *m, struct behavior *b,
                    vertex endpoint1, vertex endpoint2, long newmark)
#else /* not ANSI_DECLARATORS */
...
void delaunayfixup(struct mesh *m, struct behavior *b,
                   struct otri *fixuptri, long leftside)
#else /* not ANSI_DECLARATORS */
...
void constrainededge(struct mesh *m, struct behavior *b,
                     struct otri *starttri, vertex endpoint2, long newmark)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void formskeleton(struct mesh *m, struct behavior *b, long *segmentlist,
                  long *segmentmarkerlist, long numberofsegments)
#else /* not ANSI_DECLARATORS */
  ...
  if (b->poly) {
    ...
    for (i = 0; i < m->insegments; i++) {
      ...
      if (*stringptr == '\0') {
        ...
        end1 = (long) strtol(stringptr, &stringptr, 0);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        end2 = (long) strtol(stringptr, &stringptr, 0);
      }
      if (segmentmarkers) {
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
...
#ifdef ANSI_DECLARATORS
void carveholes(struct mesh *m, struct behavior *b, TRIREAL *holelist, long holes,
                TRIREAL *regionlist, long regions)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void splitencsegs(struct mesh *m, struct behavior *b, long triflaws)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  long acuteorg, acuteorg2, acutedest, acutedest2;
  long dummy;
  long i;
  triangle ptr;                     /* Temporary variable used by stpivot(). */
  subseg sptr;                        /* Temporary variable used by snext(). */
...

{
  ...
  if (b->poly) {
    ...
    *polyfile = fopen(polyfilename, "r");
    ...
    stringptr = readline(inputline, *polyfile, polyfilename);
    m->invertices = (long) strtol(stringptr, &stringptr, 0);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      m->mesh_dim = 2;
    } else {
      m->mesh_dim = (long) strtol(stringptr, &stringptr, 0);
    }
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      m->nextras = 0;
    } else {
      m->nextras = (long) strtol(stringptr, &stringptr, 0);
    }
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      nodemarkers = 0;
    } else {
      nodemarkers = (long) strtol(stringptr, &stringptr, 0);
    }
    if (m->invertices > 0) {
      infile = *polyfile;
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    if (nodemarkers) {
      ...
      if (*stringptr == '\0') {
        ...
        currentmarker = (long) strtol(stringptr, &stringptr, 0);
...
void transfernodes(struct mesh *m, struct behavior *b, TRIREAL *pointlist,
                   TRIREAL *pointattriblist, long *pointmarkerlist,
                   long numberofpoints, long numberofpointattribs)
#else /* not ANSI_DECLARATORS */
...
long *pointmarkerlist;
long numberofpoints;
long numberofpointattribs;
#endif /* not ANSI_DECLARATORS */
...
void readholes(struct mesh *m, struct behavior *b,
               FILE *polyfile, char *polyfilename, TRIREAL **hlist, long *holes,
               TRIREAL **rlist, long *regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  /* Read the holes. */
  stringptr = readline(inputline, polyfile, polyfilename);
  *holes = (long) strtol(stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (TRIREAL *) trimalloc(2 * *holes * (long) sizeof(TRIREAL));
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      stringptr = readline(inputline, polyfile, polyfilename);
      stringptr = findfield(stringptr);
  ...
  if ((b->regionattrib || b->vararea) && !b->refine) {
    ...
    stringptr = readline(inputline, polyfile, polyfilename);
    *regions = (long) strtol(stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (TRIREAL *) trimalloc(4 * *regions * (long) sizeof(TRIREAL));
      *rlist = regionlist;
      index = 0;
      for (i = 0; i < *regions; i++) {
        stringptr = readline(inputline, polyfile, polyfilename);
...
#ifdef ANSI_DECLARATORS
void finishfile(FILE *outfile, long argc, char **argv)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*pointlist == (TRIREAL *) NULL) {
    *pointlist = (TRIREAL *) trimalloc((long) (outvertices * 2 * sizeof(TRIREAL)));
  }
  /* Allocate memory for output vertex attributes if necessary. */
  if ((m->nextras > 0) && (*pointattriblist == (TRIREAL *) NULL)) {
    *pointattriblist = (TRIREAL *) trimalloc((long) (outvertices * m->nextras *
                                                 sizeof(TRIREAL)));
  }
  /* Allocate memory for output vertex markers if necessary. */
  if (!b->nobound && (*pointmarkerlist == (long *) NULL)) {
    *pointmarkerlist = (long *) trimalloc((long) (outvertices * sizeof(long)));
  }
  plist = *pointlist;
...
void writeelements(struct mesh *m, struct behavior *b,
                   long **trianglelist, TRIREAL **triangleattriblist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*trianglelist == (long *) NULL) {
    *trianglelist = (long *) trimalloc((long) (m->triangles.items *
                                             ((b->order + 1) * (b->order + 2) /
                                              2) * sizeof(long)));
  }
  /* Allocate memory for output triangle attributes if necessary. */
  if ((m->eextras > 0) && (*triangleattriblist == (TRIREAL *) NULL)) {
    *triangleattriblist = (TRIREAL *) trimalloc((long) (m->triangles.items *
                                                    m->eextras *
                                                    sizeof(TRIREAL)));
  }
  tlist = *trianglelist;
...
void writepoly(struct mesh *m, struct behavior *b,
               long **segmentlist, long **segmentmarkerlist)
#else /* not ANSI_DECLARATORS */
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
               TRIREAL *holelist, long holes, TRIREAL *regionlist, long regions,
               long argc, char **argv)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*segmentlist == (long *) NULL) {
    *segmentlist = (long *) trimalloc((long) (m->subsegs.items * 2 *
                                            sizeof(long)));
  }
  /* Allocate memory for output segment markers if necessary. */
  if (!b->nobound && (*segmentmarkerlist == (long *) NULL)) {
    *segmentmarkerlist = (long *) trimalloc((long) (m->subsegs.items *
                                                  sizeof(long)));
  }
  slist = *segmentlist;
...
void writeedges(struct mesh *m, struct behavior *b,
                long **edgelist, long **edgemarkerlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*edgelist == (long *) NULL) {
    *edgelist = (long *) trimalloc((long) (m->edges * 2 * sizeof(long)));
  }
  /* Allocate memory for edge markers if necessary. */
  if (!b->nobound && (*edgemarkerlist == (long *) NULL)) {
    *edgemarkerlist = (long *) trimalloc((long) (m->edges * sizeof(long)));
  }
  elist = *edgelist;
...
void writevoronoi(struct mesh *m, struct behavior *b, TRIREAL **vpointlist,
                  TRIREAL **vpointattriblist, long **vpointmarkerlist,
                  long **vedgelist, long **vedgemarkerlist, TRIREAL **vnormlist)
#else /* not ANSI_DECLARATORS */
...
long **vpointmarkerlist;
long **vedgelist;
long **vedgemarkerlist;
TRIREAL **vnormlist;
#endif /* not ANSI_DECLARATORS */
...
void writevoronoi(struct mesh *m, struct behavior *b, char *vnodefilename,
                  char *vedgefilename, long argc, char **argv)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*vpointlist == (TRIREAL *) NULL) {
    *vpointlist = (TRIREAL *) trimalloc((long) (m->triangles.items * 2 *
                                            sizeof(TRIREAL)));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (TRIREAL *) NULL) {
    *vpointattriblist = (TRIREAL *) trimalloc((long) (m->triangles.items *
                                                  m->nextras * sizeof(TRIREAL)));
  }
  *vpointmarkerlist = (long *) NULL;
  plist = *vpointlist;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    * (long *) (triangleloop.tri + 6) = (long) vnodenumber;
    triangleloop.tri = triangletraverse(m);
    vnodenumber++;
  }
  ...
  if (*vedgelist == (long *) NULL) {
    *vedgelist = (long *) trimalloc((long) (m->edges * 2 * sizeof(long)));
  }
  *vedgemarkerlist = (long *) NULL;
  /* Allocate memory for output Voronoi norms if necessary. */
  if (*vnormlist == (TRIREAL *) NULL) {
    *vnormlist = (TRIREAL *) trimalloc((long) (m->edges * 2 * sizeof(TRIREAL)));
  }
  elist = *vedgelist;
  normlist = *vnormlist;
  coordindex = 0;
...
#ifdef ANSI_DECLARATORS
void writeneighbors(struct mesh *m, struct behavior *b, long **neighborlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*neighborlist == (long *) NULL) {
    *neighborlist = (long *) trimalloc((long) (m->triangles.items * 3 *
                                             sizeof(long)));
  }
  nlist = *neighborlist;
  index = 0;
  ...
  elementnumber = b->firstnumber;
  while (triangleloop.tri != (triangle *) NULL) {
    * (long *) (triangleloop.tri + 6) = (long) elementnumber;
    triangleloop.tri = triangletraverse(m);
    elementnumber++;
  }
  * (long *) (m->dummytri + 6) = -1;
